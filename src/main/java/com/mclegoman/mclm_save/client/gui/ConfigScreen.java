...
public class ConfigScreen extends Screen {
	...
	public final void init() {
		...
		this.buttons.add(new ButtonWidget(8, this.width / 2 - 152, this.height / 6 + 120, 150, 20, "Save Block Items: " + SaveConfig.instance.saveBlockItems.value()));
		this.buttons.add(new ButtonWidget(9, this.width / 2 + 2, this.height / 6 + 120, 150, 20, "Entity In-Block Fix: " + SaveConfig.instance.blockPosFix.value()));
	...
	protected void buttonClicked(net.minecraft.client.gui.widget.ButtonWidget button) {
		if (button.active) {
			...
			if (button.id == 8) {
				...
				button.message = "Save Block Items: " + SaveConfig.instance.saveBlockItems.value();
			}
			if (button.id == 9) {
